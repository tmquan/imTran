...
class Model(ModelDesc):
    ...
    def build_graph(self, image, affnt, label):
        ...
        with tf.variable_scope('gen'):
            with tf.device('/device:GPU:0'):
                with tf.variable_scope('image2affnt'):
                    ...
                    pid = tf.pad(pia, name='pad', mode='REFLECT', paddings=[
                                                             [1*(3//2),1*(3//2)], 
                                                             [1*(3//2),1*(3//2)], 
                                                             [1*(3//2),1*(3//2)], 
                                                             [0,0]])
                    pid = Conv3D('highdim_0', 
                                  tf.expand_dims(pid, axis=0), 
                                  feature_dim, 
                                  stride=1, 
                                  kernel_shape=(3,3,3), 
                                  padding='VALID', 
                                  nl = INLReLU3D)
                    pid = residual_3d('highdim_1', 
                                   pid, 
                                   feature_dim, 
                                   first=True)
        ...
        with tf.name_scope('loss_discrim'):
            ...
            disc_loss = tf.where(tf.is_nan(disc_loss), tf.ones_like(disc_loss) * 1e+2, disc_loss);
            losses.append(1e-4*disc_loss)
        ...
        with tf.name_scope('loss_dice'):
            dice_ia = tf.identity(1.0 - dice_coe(pia, pa, axis=[0,1,2,3], loss_type='jaccard'), 
                                 name='dice_ia')  
            dice_ia = tf.where(tf.is_nan(dice_ia), tf.ones_like(dice_ia) * 1e+2, dice_ia);
        ...
        with tf.name_scope('loss_wbce'):
            ...
            malis_weighted_bce_loss = tf.where(tf.is_nan(malis_weighted_bce_loss), tf.ones_like(malis_weighted_bce_loss) * 1e+2, malis_weighted_bce_loss);
        ...
        with tf.name_scope('loss_mae'):
            ...
            mae_ia = tf.where(tf.is_nan(mae_ia), tf.ones_like(mae_ia) * 1e+2, mae_ia);
